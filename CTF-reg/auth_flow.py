...
class AuthFlow:
    ...
    def _follow_authorize_for_callback(self, start_url: str, redirect_uri: str, trace_prefix: str) -> tuple[str, str]:
        ...
        for i in range(12):
            ...
            if resp.status_code == 200:
                ...
                # 协议端需要主动选择一个 unified session，否则会停在选择页，后续
                # token exchange 只能拿到 token_exchange_user_error。
    ...
    def kickoff_otp_delivery(self, mode: str = "") -> bool:
        ...
        - 新注册: passwordless/send-otp → resend → send
        - 已有账号 / rt-only / passwordless_login: 优先 resend 当前 challenge。
          不优先 send_otp；send_otp 会新建 challenge，让旧邮件验证码失效，
          容易出现 invalid_state / wrong_email_otp_code。
        """
        mode_lc = (mode or "").strip().lower()
        is_existing = (
            bool(getattr(self, "_is_existing_account", False))
            or "existing" in mode_lc
            or "protocol_need_otp" in mode_lc
            or "protocol_verify_retry" in mode_lc
            or (getattr(self, "_existing_page_type", "") or "").lower() in ("login_password", "email_otp_verification")
            or (getattr(self, "_existing_email_verification_mode", "") or "").lower() in ("passwordless_signup", "passwordless_login")
        )

        if is_existing:
            if self.resend_otp("https://auth.openai.com/email-verification"):
                return True
            logger.warning("已有账号 resend 失败，兜底 send_otp 新建 challenge（旧邮件码会失效）")
            try:
                self.send_otp()
                return True
            except Exception as e:
                logger.warning(f"已有账号发码全 fail(mode={mode_lc or 'unknown'}): {e}")
                return False
    ...
    def _default_password_from_email(email: str) -> str:
        # New registrations should not derive the OpenAI password from the mailbox.
        # Keep the method name for existing call sites/fallbacks, but generate a
        # random >=12-character password unless LOGIN_PASSWORD explicitly supplies one.
        length = 14
        try:
            length = max(12, int(os.getenv("REG_PASSWORD_LENGTH", "14") or "14"))
        except Exception:
            length = 14
    ...
    def _choose_account_select(self, html_text: str, current_url: str) -> str:
        """处理 /choose-an-account 多账号选择页，返回下一跳 continue_url 或空串。"""
        ...
        headers["Origin"] = "https://auth.openai.com"
        candidates = [
            ("POST", "https://auth.openai.com/api/accounts/session/select", {"session_id": session_id}, "json"),
            ("POST", "https://auth.openai.com/choose-an-account", {"intent": "select", "session_id": session_id}, "form"),
        ]
    ...
    def run_protocol_login(self, mail_provider: MailProvider, email: str, password: str = "") -> AuthResult:
        ...
        # run_protocol_login 的语义就是“已有账号登录”。这里提前标记，避免
        # kickoff_otp_delivery 走新注册/passwordless send-otp，把 server-side
        # challenge state 换掉，导致邮箱抓到的旧 OTP 在 validate 时失效。
        self._is_existing_account = True
        ...
        if prefer_login_screen_first:
            try:
                ...
                if page_type == "login_password" or "/log-in/password" in (continue_url or ""):
                    ...
                    # 命中已有账号 password 路径：后续 OTP 只走 resend，避免 send_otp 新建 challenge。
                    self._is_existing_account = True
                    login_resp = self.login_password_verify(login_password)
                    ...
                    # authorize/continue 已经触发已有账号 OTP，后续只 resend 当前 challenge。
                    self._is_existing_account = True
                else:
                    logger.info(
