...
class GoPayCharger:
    ...
    def _midtrans_create_charge(self, snap_token: str) -> str:
        ...
        data = r.json()
        link = (
            data.get("gopay_verification_link_url", "")
            or data.get("deeplink_redirect", "")
            or data.get("redirect_url", "")
        )
        m = re.search(r"(?:reference|reference_id)=([A-Za-z0-9_-]+)", link)
        if not m:
            # Denied charge responses can have empty GoPay link but useful status fields.
            # Include the response body so operators can distinguish Midtrans fraud/deny
            # from parser drift.
            blob = json.dumps(data, ensure_ascii=False)
            m = re.search(r"(?:reference|reference_id)[=\": ]+([A-Za-z0-9_-]{6,})", blob)
        if not m:
            status = data.get("transaction_status") or data.get("status_code") or "unknown"
            fraud = data.get("fraud_status") or ""
            msg = data.get("status_message") or ""
            raise GoPayError(
                "midtrans charge: no GoPay verification reference "
                f"(status={status}, fraud_status={fraud}, message={msg}); "
                f"response={json.dumps(data, ensure_ascii=False)[:800]}"
            )
...
def webui_manual_otp_provider(timeout: float = 300.0, interval: float = 1.0) -> Callable[[], str]:
    """Build a WebUI/manual OTP provider.

    The WebUI runner watches the ``GOPAY_OTP_REQUEST`` marker and opens the OTP
    modal.  This is useful when GoPay sends the code by SMS or when WhatsApp
    relay is unavailable.  The code is passed back through the same legacy file
    bridge so no stdin is required.
    """
    path = Path(os.getenv("GOPAY_WEBUI_OTP_FILE") or "/tmp/gopay_webui_manual_otp.txt")
    ...
    def provider() -> str:
        try:
            ...
            pass
        print(f"GOPAY_OTP_REQUEST path={path}", flush=True)
        print("[gopay] waiting OTP from WebUI manual input (SMS/WhatsApp)", flush=True)
        deadline = time.time() + timeout
        while time.time() < deadline:
            try:
                otp = path.read_text(encoding="utf-8").strip()
            except FileNotFoundError:
                otp = ""
            except Exception:
                otp = ""
            otp = _clean_otp_candidate(otp)
            if otp:
                try:
                    path.unlink(missing_ok=True)
                except Exception:
                ...
                return otp
            time.sleep(max(0.2, interval))
        raise OTPCancelled(f"OTP timeout after {timeout}s (webui_manual file={path})")
...
def build_configured_otp_provider(
    ...
    *,
    ...
    Supported config:
      "gopay": {
        "otp": {
          "source": "http" | "file" | "command" | "manual" | "webui_manual" | "auto",
    ...
    if source in ("webui", "webui_manual", "manual_webui", "sms", "sms_manual"):
        return webui_manual_otp_provider(timeout=timeout, interval=interval)
