                ...
                if ("/email-otp" in cur or "/email-verification" in cur or "passwordless" in cur or
                    ...
                    if not otp_fetched:
                        ...
                        if filled:
                            ...
                            time.sleep(3)
                    elif "/email-verification" in cur and email_otp_stuck_since and (time.time() - email_otp_stuck_since) > 35:
                        _log("      [RT] 邮箱 OTP 提交后仍停留 email-verification >35s，提前放弃")
                        return ""
                ...
                if "/add-phone" in cur or "phone-number" in cur:
                    ...
                    for sel in [
                        ...
                        try:
                            ...
                            pass
                    # 找不到 Skip：OpenAI 强制要求 phone 验证。若配置了自动接码，则自动走手机号验证。
                    if not skipped and not getattr(page, "_addphone_gaveup", False):
                        phone_ok = False
                        try:
                            phone_ok = _rt_auto_phone_verify(page, phone_verify_cfg or {})
                        except Exception as e_phone:
                            _log(f"      [RT-SMS] 自动手机号验证异常: {type(e_phone).__name__}: {e_phone}")
                        if phone_ok:
                            page._addphone_gaveup = True
                            time.sleep(2)
                        else:
                            page._addphone_gaveup = True
                            _log("      [RT] add-phone 找不到 Skip 且自动手机号验证未完成，提前放弃避免 240s 空等")
                            break
...
def _paypal_guest_handoff_fill_nonpayment(
    ...
    *,
    ...
    with Camoufox(**_guest_camoufox_kwargs) as browser:
        ...
        if str(os.getenv("CLICK_BLUE_BUTTON", "0")).lower() in ("1", "true", "yes", "on"):
            clicked = False
            for sel in [
                ...
                try:
                    ...
                    if btn and btn.is_visible():
                        if _paypal_recaptcha_overlay_visible():
                            _log("      [Guest] reCAPTCHA overlay detected before blue button click; stop clicking")
                            break
                        _log(f"      [Guest] clicking blue button: {sel}")
                        try:
                            btn.click(timeout=5000)
                        except Exception as _e:
                            if _paypal_recaptcha_overlay_visible():
                                _log(f"      [Guest] blue button blocked by reCAPTCHA overlay: {_e}")
                                try:
                            ...
                            raise
                        clicked = True
                        _guest_pause("after blue button click", 5.5, 4.5)
                    ...
                    _log(f"      [Guest] blue button click failed {sel}: {e}")
            if not clicked:
                _log("      [Guest] blue button not found")
