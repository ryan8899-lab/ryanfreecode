...
PAYMENT_PY = CARD_DIR / "payment.py"
...
def _default_register_python() -> str:
    """Pick a Python interpreter that has the browser registration deps.

    WebUI/service launches may run pipeline.py with system python, where
    camoufox is not installed. Prefer project venv, then /root/venv, then
    current interpreter as a final fallback.
    """
    for candidate in (
...
try:
    ...
    try:
        mail.rollback_custom_mailbox()
    except Exception as rollback_exc:
        logging.warning(f"custom mail pool rollback failed: {rollback_exc}")
    raise
else:
    try:
        mail.commit_custom_mailbox()
    except Exception as commit_exc:
        logging.warning(f"custom mail pool commit failed: {commit_exc}")
...
"""
    ...
    if not python:
        python = _default_register_python()

    cmd = [python, "-c", script, auth_bundle_dir, cardw_config_path]
    print(f"[register] 注册新账号 (config={os.path.basename(cardw_config_path)}, python={python}) ...")
...
def _codex_oauth_client_id_from_card_cfg(cfg: dict) -> str:
    """Resolve Codex OAuth client_id for child payment.py processes.

    `payment.py` needs a real client_id to finish the post-payment Codex OAuth
...
def pay(card_config_path, session_token=None, access_token=None,
        ...
        gopay_otp_file=None, paypal_guest_handoff=False, python=None, timeout=600):
    """执行 Stripe 支付流程。
    ...
    if python is None:
        python = _default_register_python()
    cmd = [python, str(PAYMENT_PY), "auto",
           "--config", config_to_use, "--json-result"]
    if paypal_guest_handoff:
        cmd.append("--paypal-guest-handoff")
    elif use_paypal:
        cmd.append("--paypal")
    elif use_gopay:
        cmd.append("--gopay")
    ...
    mode_label = "paypal_guest_handoff" if paypal_guest_handoff else ("gopay" if use_gopay else ("paypal" if use_paypal else "card"))
    ...
    try:
        ...
        try:
            pay_result = pay(
                ...
                paypal_guest_handoff=paypal_guest_handoff,
                timeout=timeout_pay,
            )
            record["payment"] = {
    ...
    if is_register_only:
        ...
        for i in range(count):
            ...
            try:
                ...
                r["batch_index"] = i
                # register() returns the auth bundle, not {status:"ok"}.  The
                # old counter therefore always stayed at 0 even when accounts
                # were created successfully.
                if r.get("email"):
                    ok_count += 1
                    r.setdefault("status", "ok")
                    record["registration"] = {"status": "ok", "email": r.get("email", "")}
                else:
                    record["registration"] = {"status": "unknown"}
                _append_result(record)
            except Exception as e:
                r = {"batch_index": i, "status": "error", "error": str(e)[:200]}
                record["registration"] = {"status": "error", "error": str(e)[:500]}
                _append_result(record)
...
def _paid_or_consumed_emails() -> set[str]:
    ...
    `payment.py` writes terminal payment records to the same DB.  Some
    payment errors happen before payment.py can recover the email, so we consult
...
def pay_only(card_config_path, *, use_paypal=False, use_gopay=False,
             ...
             target_email: str = "", paypal_guest_handoff: bool = False):
    """Retry payment only.
...
def _write_rt_only_result(target: str, rt: str, result=None) -> dict:
    """Write a freshly obtained RT (and any freshed auth fields) to latest registered_accounts row."""
    ...
    try:
        ...
        if result is not None:
            for attr, col in (
                ...
                val = (getattr(result, attr, "") or "").strip()
                if val:
                    updates[col] = val
        with db._conn() as c:
            row = c.execute(
            ...
            cur = c.execute(
                f"UPDATE registered_accounts SET {set_clause} WHERE id = ?",
...
def rt_only_for_email(card_config_path: str, target_email: str) -> dict:
    ...
    # RT/Codex OAuth helpers are split out of the legacy payment module.
    ...
    try:
        ...
        print(f"[rt-only] import rt_login module 失败: {e}")
    ...
    # Ryan 指定：补 RT 先关闭纯协议路径，默认直接走浏览器；需要协议时显式 RT_PROTOCOL_DEFAULT=1。
    use_protocol_default = str(os.getenv("RT_PROTOCOL_DEFAULT", "0")).lower() not in ("0", "false", "no", "off")
    protocol_error = ""
    if use_protocol_default:
        print(f"[rt-only] 启动协议 Codex OAuth → email={target} password={'有' if account.get('password') else '无(passwordless)'}")
        old_env: dict[str, str | None] = {}
        protocol_env_defaults = {
            "LOCALAUTH_PREFER_EXISTING_ACCOUNT_LOGIN": "1",
            ...
            "OAUTH_CODEX_ADD_PHONE_REFRESH_RETRY_COUNT": "3",
            "OAUTH_CODEX_ADD_PHONE_REFRESH_SLEEP": "1.2",
        }
        for k, v in protocol_env_defaults.items():
        ...
        try:
            ...
            RegConfig = importlib.import_module("config").Config
            MailProvider = importlib.import_module("mail_provider").MailProvider
            AuthFlow = importlib.import_module("auth_flow").AuthFlow

            reg_cfg_raw = _read_card_cfg(str(reg_cfg_path)) if reg_cfg_path.exists() else {}
            reg_cfg_raw["mail"] = mail_cfg
            reg_cfg_raw["proxy"] = _build_proxy_url_from_cfg(card_cfg.get("proxy"))
            reg_cfg_raw["session_token"] = account.get("session_token", "") or ""
            reg_cfg_raw["access_token"] = account.get("access_token", "") or ""
            reg_cfg_raw["device_id"] = account.get("device_id", "") or ""
            tmp = tempfile.NamedTemporaryFile(
                mode="w", suffix=".json", prefix="rt_protocol_cfg_",
                dir=str(CARDW_DIR), delete=False,
            )
            try:
                json.dump(reg_cfg_raw, tmp, ensure_ascii=False, indent=2)
                tmp.close()
                cfg = RegConfig.from_file(tmp.name)
                mail = MailProvider(cfg.mail.catch_all_domain)
                # 默认不要在 rt-only 协议路径里从 custom_mail_pool(.used) 反查并注入
                # “邮箱取码配置”：Ryan 的补 RT 队列可能来自自配邮箱池/其它接码链路，
                # 自动覆盖 MailProvider 会走错取码源。仅在明确设置
                # RT_PROTOCOL_INJECT_CUSTOM_MAIL=1 时才启用这个兼容兜底。
                if str(os.getenv("RT_PROTOCOL_INJECT_CUSTOM_MAIL", "0")).lower() in ("1", "true", "yes", "on"):
                    try:
                        pool_candidates = []
                        env_pool = os.getenv("CUSTOM_MAIL_POOL", "").strip()
                        if env_pool:
                            pool_candidates.append(Path(env_pool))
                        base_pool = CARDW_DIR / "custom_mail_pool.txt"
                        pool_candidates.extend([base_pool, base_pool.with_suffix(base_pool.suffix + ".used")])
                            ...
                            try:
                                ...
                                for line in p.read_text(encoding="utf-8").splitlines():
                                    ...
                                    if not raw or raw.startswith("#"):
                                        continue
                                    parts = raw.split("----")
                                    if len(parts) >= 2 and _norm_email(parts[0]) == target:
                                        mail._custom_email_password = parts[1].strip()
                                        mail._custom_otp_url = parts[2].strip() if len(parts) >= 3 else ""
                                        mail._custom_pool_path = str(p)
                                        print(f"[rt-only] 协议路径已注入自定义邮箱取码配置: {target}")
                                        raise StopIteration
                    ...
                    try:
                        mail._custom_pool_path = ""
                    except Exception:
                        pass
                flow = AuthFlow(cfg)
                # 协议路径偶尔会卡在 sentinel/auth 网络请求；只给 rt-only 协议路径加
                # 超时保护，避免 WebUI 一直显示 pipeline running。超时后按原逻辑回退浏览器。
                import signal as _signal
                def _rt_protocol_timeout(_signum, _frame):
                    raise TimeoutError("rt-only protocol path timeout")
                _old_alarm_handler = _signal.getsignal(_signal.SIGALRM)
                try:
                    _timeout_s = max(30, int(os.getenv("RT_PROTOCOL_TIMEOUT", "90") or "90"))
                except Exception:
                    _timeout_s = 90
                try:
                    _signal.signal(_signal.SIGALRM, _rt_protocol_timeout)
                    _signal.alarm(_timeout_s)
                    result = flow.run_protocol_login(mail, email=target, password=account.get("password", "") or "")
                finally:
                    _signal.alarm(0)
                    try:
                        _signal.signal(_signal.SIGALRM, _old_alarm_handler)
                    except Exception:
                        pass
            finally:
                try:
                    ...
                    pass
            rt = (getattr(result, "refresh_token", "") or "").strip()
            if rt:
                print(f"[rt-only] 协议路径已获得 refresh_token (len={len(rt)})")
                return _write_rt_only_result(target, rt, result=result)
            protocol_error = "protocol_no_rt"
            print("[rt-only] 协议路径未获得 refresh_token")
        except Exception as e:
            protocol_error = f"{type(e).__name__}: {str(e)[:180]}"
            print(f"[rt-only] 协议路径异常，准备回退浏览器路径: {protocol_error}")
        finally:
    ...
    print(f"[rt-only] 启动浏览器 Codex OAuth → email={target} password={'有' if account.get('password') else '无(passwordless)'}")
    try:
        rt = _exchange_refresh_token_with_session(
            ...
            phone_verify_cfg=(card_cfg.get("rt_phone_verify") or {}),
        )
    except Exception as e:
        print(f"[rt-only] 异常: {type(e).__name__}: {str(e)[:200]}")
        return {"status": "exception", "error": str(e)[:200], "protocol_error": protocol_error, "email": target}

    if not rt:
        print(f"[rt-only] ❌ {target} 未获得 refresh_token")
        out = {"status": "no_rt", "email": target}
        if protocol_error:
            out["protocol_error"] = protocol_error
        return out

    return _write_rt_only_result(target, rt)
...
def pay_only_targets(card_config_path: str, target_emails: list[str], *,
                     ...
                     paypal_guest_handoff: bool = False) -> dict:
...
def _classify_oauth_failure(log: str) -> str:
    ...
    if "invalid_grant" in log or "no longer exists" in low or "doesn't exist" in low:
        return "account_dead"
    if "account_deactivated" in log or "deleted or deactivated" in low or "认证终止错误: account_deactivated" in log:
        return "account_deactivated"
    if "OTP 未填入" in log or "OTP 未能提交" in log or "otp_fill_failed" in log or "otp_submit_failed" in log:
        return "otp_submit_failed"
    if "邮箱 OTP 提交后仍停留 email-verification" in log:
        return "otp_stuck_after_submit"
...
def _exchange_rt_with_classification(
    ...
    """包 rt_login._exchange_refresh_token_with_session 加失败分类。
    ...
    # rt_login.py 内部会 from cf_kv_otp_provider import ...（在 CTF-reg/）
    # 要确保两个目录都在 sys.path，否则 OTP 路径 ImportError 立即返 ""。
    if str(CARDW_DIR) not in sys.path:
        sys.path.insert(0, str(CARDW_DIR))
    if str(CARD_DIR) not in sys.path:
        sys.path.insert(0, str(CARD_DIR))
    from rt_login import _exchange_refresh_token_with_session  # noqa: E402
...
def daemon(card_config_path, cardw_config_path=None, use_paypal=False):
    ...
    while not stop["flag"]:
        ...
        try:
            ...
            # 可见滑块：payment drag solver 已尝试且失败。换 IP 也救不了
...
def self_dealer(card_config_path, cardw_config_path=None, use_paypal=False,
    ...
    # RT helper now lives in CTF-pay/rt_login.py; do not import legacy payment runner.
    if str(CARD_DIR) not in sys.path:
        sys.path.insert(0, str(CARD_DIR))
    from rt_login import _exchange_refresh_token_with_session  # noqa: E402
...
def free_register_loop(card_config_path, cardw_config_path=None, count: int = 0):
    ...
    # OAuth Codex client_id：rt_login._exchange_refresh_token_with_session 读
...
示例:
  ...
  python pipeline.py --config CTF-pay/config.paypal-ip2.json --paypal --self-dealer 4
    ...
    parser.add_argument("--paypal", action="store_true",
                        help="使用 PayPal 支付")
    parser.add_argument("--paypal-guest-handoff", action="store_true",
                        help="PayPal guest/signup handoff：填非支付字段并截图停住，不填卡/CVV/密码，不创建/授权")
    ...
    if (args.paypal or args.paypal_guest_handoff) and args.gopay:
        print("[ERROR] --paypal/--paypal-guest-handoff 与 --gopay 互斥", file=sys.stderr)
        sys.exit(2)
