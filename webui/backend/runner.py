...
def build_cmd(mode: str, paypal: bool, batch: int, workers: int, self_dealer: int,
    ...
    cmd = ["xvfb-run", "-a", py, "-u", "pipeline.py",
    ...
    if register_only:
        ...
        cmd.append("--pay-only")
    if rt_only:
        # WebUI selected "补 RT" should use the dedicated RT proxy-pool runner when available.
        # It still calls pipeline.py --rt-only internally, but starts a per-account gost relay
        # so Camoufox really uses the supplied 1024 SOCKS proxy pool.
        rt_pool_runner = s.ROOT / "scripts" / "run_rt_missing_with_proxy_pool.py"
        if rt_pool_runner.exists():
            cmd = [py, "-u", str(rt_pool_runner)]
        else:
            cmd.append("--rt-only")
    if target_emails:
        joined = ",".join(e.strip() for e in target_emails if e and e.strip())
        if joined:
            if rt_only and len(cmd) >= 3 and str(cmd[-1]).endswith("run_rt_missing_with_proxy_pool.py"):
                # The proxy-pool runner reads selected emails from env; env is set in start().
                pass
            else:
...
def start(*, mode: str, paypal: bool = True, batch: int = 0, workers: int = 3,
    ...
    with _lock:
        ...
        if rt_only and target_emails:
            joined_rt = ",".join(e.strip() for e in target_emails if e and e.strip())
            if joined_rt:
                env["RT_TARGET_EMAILS"] = joined_rt
                env.setdefault("RT_LIMIT", str(len([e for e in target_emails if e and e.strip()])))
        if gopay:
