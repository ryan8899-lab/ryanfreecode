    ...
    <div class="run-body">
      ...
      <section class="run-inventory">
        ...
        <div v-if="inventory.accounts.length" class="inventory-filters">
          ...
          <select v-model="invFilters.account" class="inv-filter-sel">
            <option value="">所有账号状态</option>
            <option value="valid">正常</option>
            <option value="invalid">异常/疑封</option>
            <option value="unknown">未知</option>
        ...
        <div v-if="inventory.accounts.length" class="inventory-toolbar">
          ...
          <div class="inventory-toolbar-actions">
            <TermBtn variant="ghost" :loading="inventoryBusy" :disabled="selectedIds.size === 0" @click="checkSelectedFresh">检测选中账号状态</TermBtn>
            <TermBtn variant="ghost" :loading="inventoryBusy" :disabled="selectedIds.size === 0" @click="checkSelectedPlan">检测选中付费状态</TermBtn>
            <TermBtn variant="ghost" :loading="inventoryBusy" :disabled="selectedIds.size === 0" @click="revokeSelectedCheckout">取消权限/出库</TermBtn>
        ...
        <div v-if="filteredAccounts.length" class="inventory-list">
          <div v-for="acc in filteredAccounts" :key="acc.id || acc.email" class="inventory-row" :class="{ 'inventory-row--selected': isSelected(acc.id) }">
            <div class="inventory-row-top">
              ...
              <span class="badge" :class="planBadgeClass(acc.plan_tag)" :title="acc.live_plan_state ? `付费检测：${livePlanLabel(acc)}` : ''">{{ planLabel(acc.plan_tag) }}</span>
              <span class="badge" :class="accountStatusBadgeClass(acc.account_status || acc.last_check_status)" :title="acc.last_check_message">
                <template v-if="checkingIds.has(acc.id)">账号状态 ⟳</template>
                <template v-else>账号状态 {{ accountStatusLabel(acc.account_status || acc.last_check_status) }}</template>
              </span>
              <span class="badge" :class="payBadgeClass(acc.live_plan_state === 'free' ? 'reusable' : (acc.live_plan_state === 'paid' ? 'consumed' : acc.pay_state))">{{ payStateLabel(acc) }}</span>
...
interface InventoryAccount {
  ...
  last_check_status: "" | "valid" | "invalid" | "unknown" | "plan";
  last_check_message: string;
  last_check_at: number;
  live_plan_state: "" | "paid" | "free" | "unknown" | "missing" | string;
  live_plan_type: string;
  plan_tag: "free" | "plus" | "team" | string;
  cpa_status: string;
  cpa_pushed: boolean;
  inventory_checked_out: boolean;
  account_status: "" | "valid" | "invalid" | "unknown";
}
...
function payStateLabel(acc: InventoryAccount) {
  if (acc.live_plan_state === "free") return "可复用";
  if (acc.live_plan_state === "paid") return "已消耗";
...
// ── 库存：选择 + 验证 + 删除 ─────────────────────────────────
function accountStatusLabel(s: InventoryAccount["last_check_status"]) {
  if (s === "valid") return "正常";
  if (s === "invalid") return "异常/疑封";
  if (s === "unknown") return "未知";
  return "未检测";
}
function accountStatusBadgeClass(s: InventoryAccount["last_check_status"]) {
...
const filteredAccounts = computed<InventoryAccount[]>(() => {
  ...
  return inventory.value.accounts.filter(acc => {
    ...
    if (f.account) {
      const st = acc.account_status || acc.last_check_status || "";
      if (f.account === "unchecked") {
        if (st !== "") return false;
      } else if (st !== f.account) return false;
    }
...
function resetInvFilters() {
  invFilters.value = { search: "", plan: "", check: "", account: "", pay: "", rt: "", cpa: "" };
}
...
async function runCheck(ids: number[], label: string) {
  ...
  try {
    const r = await api.post("/inventory/accounts/check-fresh", { ids });
...
async function runPlanCheck(ids: number[], label: string) {
  if (!ids.length) { message.warning(`没有可${label}的账号`); return; }
  inventoryBusy.value = true;
  ids.forEach(id => checkingIds.value.add(id));
  try {
    const r = await api.post("/inventory/accounts/check-plan", { ids });
    const s = r.data?.summary || {};
    message.success(`${label}完成：paid=${s.paid || 0}  free=${s.free || 0}  unknown=${s.unknown || 0}`);
    await refreshInventory();
  } catch (e: any) {
    message.error(`${label}失败：${e?.response?.data?.detail || e?.message || e}`);
  } finally {
...
function checkSelectedFresh() {
  runCheck(Array.from(selectedIds.value), "实时检测选中账号状态");
}
function checkSelectedPlan() {
  runPlanCheck(Array.from(selectedIds.value), "实时检测选中付费状态");
}
...
function revokeSelectedCheckout() {
  const ids = Array.from(selectedIds.value);
  if (!ids.length) { message.warning("没有选中账号"); return; }
  const preview = emailsForIds(ids).slice(0, 5).join(", ") + (ids.length > 5 ? `, ... 共 ${ids.length} 个` : "");
  dialog.warning({
    title: "确认取消权限/出库？",
    content: () => h("div", { style: "font-size:12px; line-height:1.6" }, [
      h("div", `将 ${ids.length} 个账号从库存“已出库/权限”标记撤回，不删除账号，也不使用旧检测数据。`),
      h("div", { style: "margin-top:6px; color:#7a7363; word-break:break-all" }, preview),
    ]),
    positiveText: "确认取消",
    negativeText: "返回",
    onPositiveClick: async () => {
      inventoryBusy.value = true;
      try {
        const r = await api.post("/inventory/accounts/revoke-checkout", { ids });
        message.success(`已取消 ${r.data?.updated ?? 0} 个权限/出库标记`);
        await refreshInventory();
      } catch (e: any) {
...
function livePlanLabel(acc: InventoryAccount) {
  const plan = acc.live_plan_type ? `/${acc.live_plan_type}` : "";
  if (acc.live_plan_state === "paid") return `已付费${plan}`;
  if (acc.live_plan_state === "free") return `free${plan}`;
  if (acc.live_plan_state === "unknown") return "未知";
  if (acc.live_plan_state === "missing") return "缺账号";
  return "未检测";
}
function livePlanBadgeClass(s: string) {
  if (s === "paid") return "badge-ok";
  if (s === "free") return "badge-ghost";
  if (s === "unknown" || s === "missing") return "badge-warn";
  return "badge-ghost";
}
